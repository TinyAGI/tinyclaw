...
cp setup-wizard.sh "$BUNDLE_DIR/"
cp heartbeat-cron.sh "$BUNDLE_DIR/"
...
chmod +x "$BUNDLE_DIR/setup-wizard.sh"
chmod +x "$BUNDLE_DIR/heartbeat-cron.sh"
...
echo "  cd test-install && ./install.sh"
echo ""
